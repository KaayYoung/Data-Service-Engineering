...
def month_graph(train):
    t = train[['movie_id','revenue', 'release_month']]
...
if __name__ == '__main__':
    ...
    names_list = list(actors_mean_revenue.nlargest(50, 'mean_revenue')['actor'])
    ...
    # list_of_cast_names = list(df_training['cast'].apply(lambda x: [i['name'] for i in list(eval(x))] if x != {} else []).values)
    # top_cast_names = [m[0] for m in Counter([i for j in list_of_cast_names for i in j]).most_common(70)]
    # for g in top_cast_names:
    #     df_training['cast_name_' + g] = df_training['cast'].apply(lambda x: 1 if g in str(x) else 0)

    # for g in top_cast_names:
    #     df_validation['cast_name_' + g] = df_validation['cast'].apply(lambda x: 1 if g in str(x) else 0)
    ...
    names_list = list(directors_mean_revenue.nlargest(50, 'mean_revenue')['director'])
    ...
    names_list = list(companies_mean_revenue.nlargest(50, 'mean_revenue')['company'])
    ...
    # print(numpy.corrcoef(df_validation.revenue, pred_y)[0, 1])
