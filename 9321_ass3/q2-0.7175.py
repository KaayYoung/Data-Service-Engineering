...
if __name__ == '__main__':
    ...
    actors_list = list(actors_mean_revenue.nlargest(8, 'mean_revenue')['actor'])
    print(actors_list)
    ...
    actors_mean_revenue = pd.DataFrame.from_dict(dic, orient='index', columns=['mean_revenue']).reset_index().rename(columns={'index':'actor'})
    actors_list = list(actors_mean_revenue.nlargest(10, 'mean_revenue')['actor'])
    print(actors_list)
    for a in actors_list :
        df_training['actor_'+a]=df_training['cast_names'].apply(lambda x: 1 if a in x else 0)
    df_training = df_training.drop(['cast_names'], axis=1)
    ...
    directors_list = list(directors_mean_revenue.nlargest(15, 'mean_revenue')['director'])
    ...
    directors_mean_revenue = pd.DataFrame.from_dict(dic, orient='index', columns=['mean_revenue']).reset_index().rename(columns={'index':'director'})
    directors_list = list(directors_mean_revenue.nlargest(13, 'mean_revenue')['director'])
    ...
    companies_list = list(companies_mean_revenue.nlargest(14, 'mean_revenue')['company'])
    ...
    val_x = pd.DataFrame()
    val_x = df_validation[['runtime_cat_min_60', 'runtime_cat_61_80', 'runtime_cat_81_100', 'runtime_cat_101_120', 'runtime_cat_121_140',\
        'runtime_cat_141_170', 'runtime_cat_171_max']].select_dtypes(exclude=['object'])
    # for a in companies_list:
    ...
    summary_df.to_csv('./z5110579.PART1.output.csv', sep=',', header=True, index=True)
